...
export function Hero() {
  ...
  return (
    <section className="relative flex min-h-[100svh] items-center justify-center overflow-hidden">
      ...
      <div
        className="pointer-events-none absolute inset-4 rounded-lg border border-hero-kicker/25 sm:inset-8"
        aria-hidden="true"
      />

      <div className="container-page relative z-10 py-28 text-center text-white">
        {copy?.heroKicker && (
          <p className="animate-fade-up text-xs font-semibold uppercase tracking-[0.34em] text-hero-kicker sm:text-sm">
            {copy.heroKicker}
          </p>
        )}

        <h1
          className="mt-6 animate-fade-up font-script text-6xl leading-[1.05] text-hero-title drop-shadow-lg sm:text-7xl md:text-8xl lg:text-9xl"
          style={{ animationDelay: '80ms' }}
        >
          {content?.groomName ?? 'Peter'}
          <span className="mx-3 text-hero-kicker sm:mx-5">&amp;</span>
        ...
        >
          <span className="h-px flex-1 bg-gradient-to-r from-transparent to-hero-kicker/70" />
          <ChurchIcon className="h-5 w-5 shrink-0 text-hero-kicker" />
          <span className="h-px flex-1 bg-gradient-to-l from-transparent to-hero-kicker/70" />
        </div>

        {dateLabel && (
          <p
            className="mt-7 animate-fade-up font-heading text-xl tracking-wide text-hero-title lining-nums sm:text-2xl"
        ...
        {copy?.churchName && (
          <p
            className="mt-2 animate-fade-up text-sm uppercase tracking-[0.2em] text-hero-text/75"
        ...
        {copy?.heroTagline && (
          <p
            className="mx-auto mt-7 max-w-xl animate-fade-up text-base leading-relaxed text-hero-text/90 sm:text-lg"
        ...
        {countdown !== null && (
          <p
            className="mt-8 inline-flex animate-fade-up items-center gap-2 rounded-pill border border-hero-kicker/40 bg-black/25 px-5 py-2 text-sm font-medium text-hero-kicker backdrop-blur-sm"
        ...
        >
          ...
          <a
            href="#programme"
            className="btn w-full border border-hero-text/40 text-hero-text backdrop-blur-sm hover:bg-white/10 sm:w-auto"
          >
            {t('navProgramme')}
          </a>
        </div>

        {content?.qrPlacement === 'hero' && (
          <div
      ...
      <a
        href="#programme"
        className="absolute bottom-8 left-1/2 z-10 -translate-x-1/2 text-hero-text/60 transition-colors hover:text-hero-kicker"
